...
- **Antenna health — reported as `Unknown`, deliberately**: nothing in the operator measures antenna lock, tracking state, motor faults or misalignment, so `AntennaReady` is always `Unknown` with reason `NoAntennaProbe`. It previously reported `True` whenever the backing Node existed, which asserted a hardware fact that was never observed — alerting built on it was permanently green for the wrong reason. A real probe is tracked for v1.0 ([#68](https://github.com/thc1006/ntn-operators/issues/68)); until then treat this condition as "not measured", not as "healthy". `RFLinkHealthy` is the measured counterpart: it reports True/False only when `spec.monitoringEndpoint` gives it something to check, and is removed otherwise.
- **One satellite path per slice**: `NTNSlice.spec.satellitePath` is singular, so failover is terrestrial ⇄ *a* satellite path. Satellite-to-satellite handover, multi-constellation priority (e.g. OneWeb → Starlink → terrestrial) and failover between satellite providers are **not** supported: when the single satellite path becomes unavailable the slice can only return to terrestrial. Multi-path support is tracked in ([#49](https://github.com/thc1006/ntn-operators/issues/49)) and is currently paused.
