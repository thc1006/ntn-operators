...
### Where our code was at decision time (2026-07-06)

> ⚠ **SUPERSEDED by the 2026-07-09 Status update above.** The first two bullets described the pre-#177 state; the WS/remote-command client (`pkg/provider/ocudu/wsclient.go`) and the SatelliteEphemeris→push wiring have since **shipped** (PR #177, closing [#176]). Retained as the decision-time snapshot. The third bullet is still accurate (cell activate/deactivate is deferred to v0.7).

- *(pre-#177, now shipped)* The operator emitted **static YAML only** — `ApplyCellConfig` renders `geo_ntn.yml` into a ConfigMap; there was **no WS / remote-command client**. `PushEphemerisUpdate` was a ConfigMap **string-replace** (Phase-1) that relied on the gNB voluntarily reloading.
- *(pre-#177, now shipped)* **`SatelliteEphemeris` was not wired to OCUDU:** it fetched GP/OMM and ran SGP4 (`pkg/ephemeris/propagator.go`) but the propagated ECEF was used **only for pass prediction and discarded**; `ephemerisRef` merely re-pushed the NTNCellConfig's own static ephemeris. This was the concrete first gap ([#176], now closed).
- **`NTNCellConfig` has no activate/deactivate / cell-lock** — "deactivate" today = delete the CR → ConfigMap removed. (Still accurate; deferred to v0.7.)
